...
import { Dialog, DialogTitle, DialogContent, Grid, Avatar, DialogActions, Button } from '@mui/material';
...
const AvatarSelectionDialog: React.FC<AvatarSelectionDialogProps> = ({ open, onClose, onSelect, onConfirm }) => {
  ...
  return (
    ...
    PaperProps={{
        sx: { 
            backgroundColor: 'rgba(235, 200, 255, 0.8)', borderRadius: '50px 50px 50px 50px' 
        }
      }}
    >
      <DialogTitle
          ...
          {avatars.map((avatar) => (
            <Grid item xs={4} key={avatar}>
              <Avatar
                ...
                border: selectedAvatar === avatar ? '2px solid purple' : 'none',
                borderRadius: '50%' }}
      ...
      <DialogActions>
        ...
        <Button onClick={handleConfirm} disabled={!selectedAvatar}>Confirm</Button>
      </DialogActions>
    </Dialog>
  );
};
