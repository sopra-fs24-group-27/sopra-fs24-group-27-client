  ...
  return (
    <BaseContainer className="game container">
      ...
      <div className="game user-list">
        {players &&
          players.map((player) => <Player key={player.id} user={player} />)}
      </div>
      <div className="game-details">
            ...
            gameState
              ...
              .map((spy, index) => (
                <div key={spy.user.id || index}>
                  ...
                  <p>
                    {spy.songInfo.title} by {spy.songInfo.artist}
                  </p>
                  <img
                    src={spy.songInfo.imageUrl}
                    alt={`Cover of ${spy.songInfo.title}`}
                    style={{
                      width: "200px",
                      display: "block",
                      margin: "0 auto",
        ...
        <div className="game-column">
          ...
          {gameState && (
            <div>
              <p>
                {gameState[0].songInfo.title} by {gameState[0].songInfo.artist}
              </p>
              <img
                src={gameState[0].songInfo.imageUrl}
                alt={`Cover of ${gameState[0].songInfo.title}`}
                style={{ width: "200px", display: "block", margin: "0 auto" }}
              />
              <a
                href={gameState[0].songInfo.playUrl}
                target="_blank"
                rel="noopener noreferrer"
              >
                Listen to Song
