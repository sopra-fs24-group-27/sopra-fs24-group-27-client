...
export default function Wait() {
  ...
  const [roomInfo, setRoomInfo] = useState({
    hostId: null,
    settings: { artist: null, genre: "", market: "" },
    players: [],
  });
    ...
    const fetchGameData = async () => {
      try {
        ...
        const currentPlayer = response.data.players.find(
          (p) => p.user.id === parseInt(currentUserId, 10)
        );
        if (currentPlayer) {
      ...
      <Container component="main" maxWidth="md">
        ...
        >
          ...
          <Typography variant="subtitle1" component="h3">
            Artist {roomInfo.settings.artist}
          </Typography>
          ...
          <Grid item>
            {roomInfo?.players &&
              roomInfo.players.map((player, index) => (
                ...
                >
                  <PlayerCard
                    user={{ ...player.user, scores: player.score ?? 0 }}
                  />
                </div>
              ))}
          </Grid>
          <Grid item>
            <Button
              ...
              onClick={startGame}
              disabled={
                currentUser.id !== hostUser.id || roomInfo.players.length !== 4
              }
            >
              Start
            </Button>
