...
class ApiClient {
  ...
  async getGameStauts(gameId) {
    try {
      ...
      console.error(
        `Error when getting game status:\n ${this.handleRequestError(error)}`
      );
      throw error;
    }
  }
  ...
  async join(gameId, playerId) {
    try {
      const response = await this.client.post(
        `/games/${gameId}/join?playerId=${playerId}`
      );
  ...
  async quit(gameId, playerId) {
    try {
      const response = await this.client.post(
        `/games/${gameId}/quit?playerId=${playerId}`
      );

      return response.data;
    } catch (error) {
      console.error(
        `Error when leaving room:\n ${this.handleRequestError(error)}`
      );
      throw error;
    }
  }
  ...
  async listen(gameId, playerId) {
    try {
      const response = await this.client.post(
        `/games/${gameId}/listen?playerId=${playerId}`
      );
  ...
  async newEmojiRound(gameId) {
    try {
      ...
      console.error(
        `Error when starting new emoji description round:\n ${this.handleRequestError(error)}`
      );
      throw error;
    }
  }
  ...
  async emoji(gameId, playerId, requestBody) {
    try {
      const response = await this.client.post(
        `/games/${gameId}/emoji?playerId=${playerId}`,
        requestBody
      );
  ...
  async vote(gameId, playerId, requestBody) {
    try {
      const response = await this.client.post(
        `/games/${gameId}/vote?playerId=${playerId}`,
        requestBody
      );
